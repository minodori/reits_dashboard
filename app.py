...
형태_선택 = st.sidebar.multiselect(
    "분양 형태",
    options=df['형태'].unique(),
    default=df['형태'].unique()
)

# 시공사 필터
시공사_선택 = st.sidebar.multiselect(
    "시공사",
    options=df['시공사'].unique(),
    default=df['시공사'].unique()
)
...
with col3:
    공공_비율 = (df_filtered[df_filtered['형태']=='공공 분양']['세대수'].sum() / 
                df_filtered['세대수'].sum() * 100)
...
# 준공 월별 집계
df_pivot = df_filtered.groupby(['단지', pd.Grouper(key='준공', freq='M')])['세대수'].sum().reset_index()
...
with col2:
    ...
    시공사별 = df_filtered.groupby('시공사')['세대수'].sum().sort_values(ascending=True).reset_index()
...
# 표시할 컬럼만 선택
display_df = df_filtered[['단지', '블럭', '형태', '분양', '준공', '전매', 
                           '시행사', '시공사', '면적', '세대수']].copy()
